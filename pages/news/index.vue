<template>
    <div class="container" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy" infinite-scroll-distance="20">
            ...
            <div>
                ...
                <el-tooltip class="item" effect="light" content="发布信息需要先登录" placement="top" :disabled="isLogin">
                    <div style="float: right;">
                        <el-button type="primary" round plain :disabled="!isLogin" @click="showEdit=!showEdit">发布新闻</el-button>
                    </div>
                </el-tooltip>
            </div>
        </div>
            ...
            <el-collapse-transition>
                <div v-show="showEdit"> 
                    ...
                    <el-form-item label="标题" prop="title">
                        <el-input v-model="data.title" placeholder="请输入新闻标题" style="margin-bottom: 10px;width: 400px;margin-right: 400px;"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="内容" prop="content">
                        <el-input v-model="data.content" style="display:none;"/>
        ...
        <div>
            <el-row :gutter="12">
			<el-col :span="8" v-for="(item,index) in newsData" :key="index">
				<el-card shadow="hover" style="margin-bottom: 10px;height: 550px;" :body-style="{ padding: '0px' }" >
                    ...
                    <div style="float: right;margin: 0 10px 10px 0;">
                        <a :href="'/news/item/'+item.id" target="_blank">
...
export default {
    ...
    asyncData(){
        return newsApi.search(1,6,{state:'1'}).then(res=>{
            return {newsData: res.data.data.rows}
        })
    },
    data(){
        // 表单验证函数
        ...
        var validatePicture = (rule, value, callback) => {
            ...
            if (this.$refs.picture.uploadFiles.length === 0) {
                callback(new Error('请选择新闻封面'));
            }
            callback();  
        };
        var validateIntro = (rule, value, callback) => {
            if (value === undefined || value === '') {
            callback(new Error('请输入新闻简介'));
            }
            callback();  
        };
    ...
    methods: {
        ...
        uploadFileMethod(param){
            ...
            newsApi.UploadFile(formData).then(response=>{
                if (response) {
        ...
        submit(formName){
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    setTimeout(()=>{
                        newsApi.addNews(this.data).then(res=>{
                            this.$message({
        ...
        loadMore(){
          ...
          this.currentPage++;
            newsApi.search(this.currentPage,6,{state:'1'}).then(res=>{
            this.newsData = this.newsData.concat(res.data.data.rows)
            })
           this.busy = false;
        },
