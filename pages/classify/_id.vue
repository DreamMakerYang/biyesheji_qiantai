...
export default {
	...
	asyncData({params}){
        ...
        return rubbishApi.search(1,8,{optionsValue: params.id,state: '1'}).then(res=>{
	...
	mounted(){
		switch ( this.$route.params.id ){
			case '1':this.title='可回收垃圾';break;
			case '2':this.title='湿垃圾';break;
			case '3':this.title='干垃圾';break;
			case '4':this.title='有害垃圾';break;
		}
		// alert(this.title);
	},
	...
	data(){
		...
		return{
            ...
            searchMap: {
                // 选择器值
                optionsValue: this.$route.params.id,
                // 已通过状态
                state: '1'
			},
			rules: {
